...
-- $ for target in `ls test/|xargs -n1 basename|sed 's/\.cpp//g'`;do echo target\(\"$target\"\);echo "    "set_kind\(\"binary\"\); echo "    "add_files\(\"test/${target}.cpp\"\); echo "    "add_deps\(\"routingkit\"\);done;
...
-- $ for target in `ls examples/|xargs -n1 basename|sed 's/\.cpp//g'`;do echo target\(\"$target\"\);echo "    "set_kind\(\"binary\"\); echo "    "add_files\(\"examples/${target}.cpp\"\); echo "    "add_deps\(\"routingkit\"\);done;
